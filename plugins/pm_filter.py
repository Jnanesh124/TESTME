...
from pyrogram.errors.exceptions.bad_request_400 import MediaEmpty, PhotoInvalidDimensions
from utils import get_size, is_subscribed, pub_is_subscribed, get_poster, search_gagala, temp, get_settings, save_group_settings, get_shortlink, get_tutorial, send_all, get_cap, clean_filename
...
async def next_page(bot, query):
    ...
    # Calculate next offset based on settings
    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0 or n_offset == "":
                ...
                btn.append([InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(offset/5)+1} / {math.ceil(total_results/5)}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("⌫ 𝐁𝐀𝐂𝐊", callback_data=f"next_{req}_{key}_{off_set}"),
                        InlineKeyboardButton(f"{math.ceil(offset/5)+1} / {math.ceil(total_results/5)}", callback_data="pages"),
            ...
            if n_offset == 0 or n_offset == "":
                ...
                btn.append([InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(offset/max_btn)+1} / {math.ceil(total_results/max_btn)}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("⌫ 𝐁𝐀𝐂𝐊", callback_data=f"next_{req}_{key}_{off_set}"),
                        InlineKeyboardButton(f"{math.ceil(offset/max_btn)+1} / {math.ceil(total_results/max_btn)}", callback_data="pages"),
                        InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")
                    ],
                )
    except KeyError:
...
async def send_files_cb_handler(client: Client, query: CallbackQuery):
...
@Client.on_callback_query(filters.regex(r"^unmuteme"))
async def unmute_me_cb(client: Client, query: CallbackQuery):
...
async def check_sub_cb(client: Client, query: CallbackQuery):
    if AUTH_CHANNEL and not await is_subscribed(client, query):
        await query.answer("Jᴏɪɴ ᴏᴜʀ Bᴀᴄᴋ-ᴜᴘ ᴄʜᴀɴɴᴇL ᴍᴀʜɴ! 😒", show_alert=True)
        return
    ident, kk, file_id = query.data.split("#")
    await query.answer(url=f"https://t.me/{temp.U_NAME}?start={kk}_{file_id}")

@Client.on_callback_query(filters.regex(r"^pages"))
async def pages_cb(client: Client, query: CallbackQuery):
    await query.answer()

@Client.on_callback_query(filters.regex(r"^send_fsall"))
async def send_fsall_cb(client: Client, query: CallbackQuery):
...
async def send_fall_cb(client: Client, query: CallbackQuery):
    temp_var, ident, key, offset = query.data.split("#")
    search = FRESH.get(key)
 #   if not search:
   #     await query.answer(script.OLD_ALRT_TXT.format(query.from_user.first_name),show_alert=True)
...
async def kill_files_dq_cb(client: Client, query: CallbackQuery):
...
async def open_settings_grp_cb(client: Client, query: CallbackQuery):
    ...
    if (
        ...
        await query.answer("Yᴏᴜ Dᴏɴ'ᴛ Hᴀᴠᴇ Tʜᴇ Rɪɢʜts Tᴏ Dᴏ Tʜɪs !", show_alert=True)
        return
...
async def open_settings_pm_cb(client: Client, query: CallbackQuery):
    ident, grp_id = query.data.split("#")
    userid = query.from_user.id if query.from_user else None
    st = await client.get_chat_member(grp_id, userid)
    if (
...
async def show_option_cb(client: Client, query: CallbackQuery):
    ...
    if query.from_user.id in ADMINS:
        ...
        await query.answer("Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢʜts ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)

@Client.on_callback_query(filters.regex(r"^unavailable"))
async def unavailable_cb(client: Client, query: CallbackQuery):
    ident, from_user = query.data.split("#")
    btn = [[
...
async def uploaded_cb(client: Client, query: CallbackQuery):
    ident, from_user = query.data.split("#")
    btn = [[
            InlineKeyboardButton("✅ Uᴘʟᴏᴀᴅᴇᴅ ✅", callback_data=f"upalert#{from_user}")
          ]]
    btn2 = [[
             ...
             InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜp Lɪɴᴋ", url="https://t.me/+KzbVzahVdqQ3MmM1")
           ]]
    if query.from_user.id in ADMINS:
        user = await client.get_users(from_user)
        reply_markup = InlineKeyboardMarkup(btn)
...
async def already_available_cb(client: Client, query: CallbackQuery):
    ident, from_user = query.data.split("#")
    btn = [[
        InlineKeyboardButton("🟢 Aʟʀᴇᴀᴅʏ Aᴠᴀɪʟᴀʙʟᴇ 🟢", callback_data=f"alalert#{from_user}")
    ]]
    btn2 = [[
        ...
        InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜp Lɪɴᴋ", url="https://t.me/vj_bots")
    ]]
    if query.from_user.id in ADMINS:
        user = await client.get_users(from_user)
        reply_markup = InlineKeyboardMarkup(btn)
...
async def alalert_cb(client: Client, query: CallbackQuery):
    ident, from_user = query.data.split("#")
...
async def upalert_cb(client: Client, query: CallbackQuery):
    ident, from_user = query.data.split("#")
    if int(query.from_user.id) == int(from_user):
        user = await client.get_users(from_user)
...
async def unalert_cb(client: Client, query: CallbackQuery):
    ident, from_user = query.data.split("#")
    if int(query.from_user.id) == int(from_user):
        user = await client.get_users(from_user)
...
async def generate_stream_link_cb(client: Client, query: CallbackQuery):
    ...
    if query.data == "reqinfo":
        ...
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [[
                ...
                InlineKeyboardButton('ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇL', url=CHNL_LNK)
            ]]
        else:
            buttons = [[
                InlineKeyboardButton('⤬ ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘ ⤬', url=f'http://t.me/{temp.U_NAME}?startgroup=true')
        ...
        buttons = [[
            InlineKeyboardButton(' বহুম ʙᴀᴄᴋ', callback_data='start')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        buttons = [[
            ...
            InlineKeyboardButton(' বহুম Bᴀᴄᴋ', callback_data='help'),
        ...
        buttons = [[
            InlineKeyboardButton(' বহুম Bᴀᴄᴋ', callback_data='filters')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        buttons = [[
            InlineKeyboardButton(' বহুম Bᴀᴄᴋ', callback_data='manuelfilter')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        buttons = [[
            InlineKeyboardButton(' বহুম Bᴀᴄᴋ', callback_data='admin')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        btn = [[
            ...
            InlineKeyboardButton(" বহুম Bᴀᴄᴋ", callback_data="start"),
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                btn.append(
                    [InlineKeyboardButton("⌫ 𝐁𝐀𝐂𝐊", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"{math.ceil(int(offset)/5)+1} / {math.ceil(total_results/5)}", callback_data="pages")]
                )
            elif off_set is None:
                btn.append([InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/5)+1} / {math.ceil(total_results/5)}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("⌫ 𝐁𝐀𝐂𝐊", callback_data=f"next_{req}_{key}_{off_set}"),
                        InlineKeyboardButton(f"{math.ceil(int(offset)/5)+1} / {math.ceil(total_results/5)}", callback_data="pages"),
                        InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")
                    ],
                )
        else:
            ...
            if n_offset == 0:
                btn.append(
                    [InlineKeyboardButton("⌫ 𝐁𝐀𝐂𝐊", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total_results/int(MAX_B_TN))}", callback_data="pages")]
                )
            elif off_set is None:
                btn.append([InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total_results/int(MAX_B_TN))}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("⌫ 𝐁𝐀𝐂𝐊", callback_data=f"next_{req}_{key}_{off_set}"),
                        InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total_results/int(MAX_B_TN))}", callback_data="pages"),
                        InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")
                    ],
                )
    except KeyError:
        ...
        try:
            ...
            try:
                ...
                if settings['auto_delete']:
                    ...
                    await hehe.delete()
                    await message.delete()
        except (MediaEmpty, PhotoInvalidDimensions):
            pic = imdb.get('poster')
