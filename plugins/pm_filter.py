...
async def filter_ignored_words(filename):
    """Remove ignored words from filename"""
    try:
        ignored_words = await get_ignored_words()
        ...
        filtered_words = []
        for word in words:
            # Check if word (without special characters) is in ignored list
            clean_word = word.lower().strip('[]()@.')
            if clean_word not in [iw.lower() for iw in ignored_words]:
                filtered_words.append(word)

        return ' '.join(filtered_words) if filtered_words else filename
    except Exception as e:
        logger.exception('Error filtering ignored words!', exc_info=True)
        return filename
        ...
        if manual == False:
            ...
            try:
                if settings['auto_ffilter']:
                    ai_search = True
                    reply_msg = await message.reply_text(f"<b>Searching : {message.text} </b>")
...
async def pm_text(bot, message):
    ...
    # Skip if the message contains "www." or "https"
    if "www." in content or "https" in content:
        return

    # Skip if the message starts with "/" or "#"
    if content.startswith("/") or content.startswith("#"):
        return  # ignore commands and hashtags

    if PM_SEARCH:
        ai_search = True
        reply_msg = await bot.send_message(message.from_user.id, f"<b>Searching : {content} </b>", reply_to_message_id=message.id)
...
async def next_page(bot, query):
   ...
   # if not search:
    ...
    if settings["button"]:
        btn = []
        for file in files:
            # Apply ignored words filter
            filtered_filename = await filter_ignored_words(file['file_name'])
            # Apply existing filters
            display_name = ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), filtered_filename.split()))
    ...
    try:
        if settings['max_btn']:
            max_btn_count = 5  # Fixed to 5 as you requested
            if 0 < offset <= max_btn_count:
                off_set = 0
            elif offset == 0:
                off_set = None
            else:
                off_set = offset - max_btn_count
            if n_offset == 0:
                btn.append([InlineKeyboardButton("⌫ 𝐁𝐀𝐂𝐊", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"{math.ceil(int(offset)/max_btn_count)+1} / {math.ceil(total/max_btn_count)}", callback_data="pages")])
            elif off_set is None:
                btn.append([InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/max_btn_count)+1} / {math.ceil(total/max_btn_count)}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append([
                    InlineKeyboardButton("⌫ 𝐁𝐀𝐂𝐊", callback_data=f"next_{req}_{key}_{off_set}"),
                    InlineKeyboardButton(f"{math.ceil(int(offset)/max_btn_count)+1} / {math.ceil(total/max_btn_count)}", callback_data="pages"),
            ...
            if n_offset == 0:
                btn.append([InlineKeyboardButton("⌫ 𝐁𝐀𝐂𝐊", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages")])
            elif off_set is None:
...
async def advantage_spoll_choker(bot, query):
   ...
   # if not movies:
    ...
    gl = await global_filters(bot, query.message.reply_to_message, text=movie)
    if gl == False:
        k = await manual_filters(bot, query.message.reply_to_message, text=movie)
        if k == False:
            files, offset, total_results = await get_search_results(query.message.reply_to_message.chat.id, movie, offset=0, filter=True)
            if files:
                k = (movie, files, offset, total_results)
                ai_search = True
                reply_msg = await query.message.edit_text(f"<b>Searching : {movie} </b>")
                await auto_filter(bot, movie, query.message.reply_to_message, reply_msg, ai_search, k)
            else:
...
async def filter_yearss_cb_handler(client: Client, query: CallbackQuery):
    ...
    if offset != "":
        try:
            if settings['max_btn']:
                btn.append([InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪",callback_data=f"next_{req}_{key}_{offset}")])

            else:
                btn.append([InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪",callback_data=f"next_{req}_{key}_{offset}")])
        except KeyError:
            await save_group_settings(query.message.chat.id, 'max_btn', True)
            btn.append([InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪",callback_data=f"next_{req}_{key}_{offset}")])
    else:
...
async def languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="👇 s𝖾𝗅𝖾𝖼ᴛ 𝖸𝗈𝗎𝗋 𝖫𝖺𝗇𝗀𝗎𝖺𝗀𝖾𝗌 👇", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
    ...
    if offset != "":
        try:
            if settings['max_btn']:
                ...
                btn.append([InlineKeyboardButton("ᴘᴀɢᴇ", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="ɴᴇxᴛ ⇛",callback_data=f"next_{req}_{key}_{offset}")])
        except KeyError:
            await save_group_settings(query.message.chat.id, 'max_btn', True)
            btn.append([InlineKeyboardButton("ᴘᴀɢᴇ", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="ɴᴇxᴛ ⇛",callback_data=f"next_{req}_{key}_{offset}")])
    else:
...
async def delete_file(client: Client, query: CallbackQuery):
    ...
    # Check verification if VERIFY is enabled
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜesᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/+KzbVzahVdqQ3MmM1")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜesᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/vj_bots")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        buttons = [[
            InlineKeyboardButton('IM Bᴀᴄᴋ', callback_data='start')
        ]]
        await client.edit_message_media(
            query.message.chat.id,
            query.message.id,
        ...
        buttons = [[
            ...
            InlineKeyboardButton('IM Bᴀᴄᴋ', callback_data='help'),
        ...
        buttons = [[
            InlineKeyboardButton('IM Bᴀᴄᴋ', callback_data='filters')
        ]]
        await client.edit_message_media(
            query.message.chat.id,
            query.message.id,
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Sᴏᴜʀᴄᴇ Cᴏᴅᴇ', url="https://github.com/V")
        ],[
        ...
        buttons = [[
            InlineKeyboardButton('IM Bᴀᴄᴋ', callback_data='manuelfilter')
        ]]
        await client.edit_message_media(
            query.message.chat.id,
            query.message.id,
        ...
        buttons = [[
            InlineKeyboardButton('IM Bᴀᴄᴋ', callback_data='admin')
        ]]
        await client.edit_message_media(
            query.message.chat.id,
            query.message.id,
        ...
        btn = [[
            InlineKeyboardButton("IM Bᴀᴄᴋ", callback_data="help"),
            InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/JNK_BACKUP")
        ]]
        await client.edit_message_media(
            query.message.chat.id,
            query.message.id,
        ...
        btn = [[
            InlineKeyboardButton("IM Bᴀᴄᴋ", callback_data="start"),
            InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/ROCKERSBAKUP")
        ]]
        await client.edit_message_media(
            query.message.chat.id,
            query.message.id,
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    if offset != "":
        try:
            if settings['max_btn']:
                max_btn_count = 5  # Fixed to 5 as you requested
                btn.append([InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/max_btn_count)}",callback_data="pages"), InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪",callback_data=f"next_{req}_{key}_{offset}")])
            else:
                btn.append([InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪",callback_data=f"next_{req}_{key}_{offset}")])
        except KeyError:
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    if AI_SPELL_CHECK == True and vj_search == True:
        vj_search_new = False
        vj_ai_msg = await reply_msg.edit_text("<b>I Am Trying To Find Your Movie With Your Wrong Spelling</b>")
        movienamelist = []
